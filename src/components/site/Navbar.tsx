...
export function Navbar() {
  ...
  return (
    ...
    >
      ...
      {open && (
        <div className="fixed inset-0 z-[60] flex flex-col items-center justify-center bg-white lg:hidden">
          <button
            aria-label="Menü schließen"
            className="absolute right-4 top-5 rounded-md p-2"
          ...
          <nav className="flex flex-col items-center gap-2">
            {NAV_LINKS.map((l) => (
              <Link
                key={l.to}
                to={l.to}
                className="px-4 py-3 text-2xl font-semibold text-foreground/80 transition-colors hover:text-foreground"
                activeProps={{ className: "text-primary" }}
                activeOptions={{ exact: l.to === "/" }}
              >
                {l.label}
            ...
            <Link
              to="/kontakt"
              className="mt-4 inline-flex items-center justify-center rounded-full bg-primary px-8 py-4 text-base font-bold text-primary-foreground"
            >
              Jetzt anmelden
            </Link>
          </nav>
